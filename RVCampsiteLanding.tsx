...
interface Campground {
  id: string;
  name: string;
  location: { address: { city: string; state_code: string } };
  price: { minimum: number };
  amenities: {
...
const CampgroundCard: React.FC<CampgroundCardProps> = ({ camp }) => {
  ...
  return (
    <article style={{ border: '1px solid #e0e0e0', borderRadius: '8px', background: 'white', overflow: 'hidden' }}>
      <div style={{ height: '160px', background: 'linear-gradient(135deg, #4a90e2 0%, #2c5aa0 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '12px', fontWeight: '500' }}>Photo</div>
      <div style={{ padding: '1.25rem' }}>
        ...
        <button style={{ width: '100%', padding: '0.625rem', marginTop: '0.75rem', background: '#1b7f5e', border: 'none', color: 'white', borderRadius: '4px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>View Availability</button>
      </div>
    </article>
  );
};
...
export default function RVCampsiteLanding() {
      ...
      const mockData: Campground[] = [
        {
          ...
          location: { address: { city: 'Tahoe City', state_code: 'CA' } },
          price: { minimum: 45 },
          amenities: { electric_hookups: true, water_hookups: true, sewer_hookups: true, pets_allowed: true, wifi: true, showers: true },
          big_rig_friendly: true,
          has_pull_through_sites: true,
          short_description: 'Beautiful lakefront RV park with full amenities and water access.',
        },
        {
          id: '2',
          name: 'Mountain View Campgrounds',
          location: { address: { city: 'South Lake Tahoe', state_code: 'CA' } },
          price: { minimum: 55 },
          amenities: { electric_hookups: true, water_hookups: true, sewer_hookups: true, dump_station: true, showers: true },
          short_description: 'Scenic mountain location with full hookups and stunning views.',
        },
        {
          id: '3',
          name: 'Riverside Haven',
          location: { address: { city: 'Truckee', state_code: 'CA' } },
          price: { minimum: 38 },
          amenities: { electric_hookups: true, water_hookups: true, sewer_hookups: true, pets_allowed: true, dump_station: true, showers: true, wifi: true },
          big_rig_friendly: true,
          has_pull_through_sites: true,
          short_description: 'River-access camping with excellent facilities and pet-friendly policies.',
        },
      ];

      setCampgrounds(mockData);
      setIsLoading(false);
      setHasSearched(true);
    }, 1200);
  };
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Hero */}
      <section style={{ position: 'relative', height: '550px', background: 'url("https://images.unsplash.com/photo-1478131143081-80f7f84ca84d?w=1400&h=600&fit=crop") center/cover no-repeat', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(26, 58, 52, 0.2)' }}></div>
        <h2 style={{ position: 'relative', zIndex: 1, fontSize: '56px', fontWeight: '700', color: 'white', margin: '0', lineHeight: '1.2', textAlign: 'center', textShadow: '0 2px 8px rgba(0,0,0,0.4)' }}>Find Your Adventure</h2>
      </section>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <form onSubmit={handleSearch} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', marginBottom: '1.5rem' }}>
            ...
            <div>
              ...
              <input type="date" value={checkIn} onChange={(e) => setCheckIn(e.target.value)} style={{ width: '100%', padding: '0.75rem', border: '1px solid #d0d0d0', borderRadius: '4px', fontSize: '13px', color: '#1a3a34', background: 'white' }} />
            </div>
            <div>
              ...
              <input type="date" value={checkOut} onChange={(e) => setCheckOut(e.target.value)} style={{ width: '100%', padding: '0.75rem', border: '1px solid #d0d0d0', borderRadius: '4px', fontSize: '13px', color: '#1a3a34', background: 'white' }} />
            </div>
            <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
              <button type="submit" style={{ padding: '0.75rem', background: '#1b7f5e', color: 'white', border: 'none', borderRadius: '4px', fontWeight: '700', fontSize: '13px', cursor: 'pointer', width: '100%' }}>Search</button>
            </div>
          </form>
          ...
          {isLoading && (
            <div style={{ textAlign: 'center', padding: '2rem' }}>
              <p style={{ fontSize: '14px', color: '#6b8b84' }}>Searching...</p>
            </div>
          )}
          ...
          {hasSearched && !isLoading && !error && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.5rem' }}>
                {campgrounds.map(camp => <CampgroundCard key={camp.id} camp={camp} />)}
              </div>
            </div>
          )}
        </div>
